...
const jogosAtivos: Map<string, JogoVelha> = new Map();
...
function renderTabuleiro(tabuleiro: string[]) {
  let str = "";
  for (let i = 0; i < 9; i++) {
...
function verificarVencedor(tabuleiro: string[]) {
  const combinacoes = [
...
export async function comandoVelha(pico: any, from: string, messageDetails: any) {
  const msg = messageDetails.message;
  const text = msg?.conversation || msg?.extendedTextMessage?.text || "";
  const mencionados = msg?.extendedTextMessage?.contextInfo?.mentionedJid || [];

  if (!text.startsWith(".velha") || mencionados.length === 0) {
    await pico.sendMessage(from, { text: "Marque o adversário: .velha @usuario" });
    return;
  }
  ...
  if (desafiante === adversario) {
    await pico.sendMessage(from, { text: "Você não pode jogar contra si mesmo!" });
    return;
  }
  ...
  for (const jogo of jogosAtivos.values()) {
    if ((jogo.jogadorX === desafiante && jogo.jogadorO === adversario) ||
      ...
      await pico.sendMessage(from, { text: "Já existe um jogo ativo entre vocês!" });
      return;
    }
  }
...
// Aceitar desafio
export async function aceitarVelha(pico: any, from: string, messageDetails: any) {
  const msg = messageDetails.message;
  const text = msg?.conversation?.toLowerCase() || msg?.extendedTextMessage?.text?.toLowerCase() || "";
  const participante = messageDetails.key.participant || messageDetails.key.remoteJid;

  if (!["aceitar", "sim", "👍"].includes(text)) return;
...
// Jogar no tabuleiro
export async function velha(pico: any, from: string, messageDetails: any) {
  const msg = messageDetails.message;
  const text = msg?.conversation || msg?.extendedTextMessage?.text || "";
  const participante = messageDetails.key.participant || messageDetails.key.remoteJid;
  ...
  if ((jogo.turno === "X" && participante !== jogo.jogadorX) ||
    ...
    await pico.sendMessage(from, { text: "Não é seu turno!" });
    return;
  }

  if (jogo.tabuleiro[pos] !== "") {
    await pico.sendMessage(from, { text: "Posição já ocupada!" });
    return;
  }
